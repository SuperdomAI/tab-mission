...
export default function WeeklyReport({ open, onClose }: WeeklyReportProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`drawer-panel fixed top-0 right-0 h-full w-[28rem] z-[70] bg-popover border-l border-hairline shadow-2xl flex flex-col ${
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-hairline">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="font-semibold text-ink text-base">Weekly Report</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6">
          ...
          <section>
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="bg-surface rounded-xl p-3 text-center border border-hairline">
                <div className="text-2xl font-mono text-ink tabular-nums">
                  {totalOpened}
                </div>
                <div className="text-xs text-faint mt-0.5">Opened</div>
              </div>
              <div className="bg-surface rounded-xl p-3 text-center border border-hairline">
                <div className="text-2xl font-mono text-ink tabular-nums">
                  {totalClosed}
                </div>
                <div className="text-xs text-faint mt-0.5">Closed</div>
              </div>
              <div className="bg-surface rounded-xl p-3 text-center border border-hairline">
                <div className="text-2xl font-mono text-ink tabular-nums">
                  {msToHours(totalTrackedMs)}h
                </div>
                <div className="text-xs text-faint mt-0.5">Tracked</div>
              </div>
            </div>
          </section>
          ...
          {topDomain && (
            <section>
              <h3 className="label-mono mb-3">Most Visited Domain</h3>
          ...
          {/* Daily bar chart */}
          <section>
            <h3 className="label-mono mb-3">Daily Tabs Opened</h3>
            ...
            <div className="flex gap-3 mt-2 text-xs text-faint">
              <span className="flex items-center gap-1">
                <span className="w-2 h-2 rounded-sm bg-status-stale inline-block" />{" "}
                Busiest
              </span>
              <span className="flex items-center gap-1">
                <span className="w-2 h-2 rounded-sm bg-status-recent inline-block" />{" "}
                Most closed
              </span>
            </div>
          </section>
                  ...
                  <div key={day.date}>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {top3.map(({ domain, ms }) => (
                        <span
                          key={domain}
                          className="text-xs bg-surface text-muted px-2 py-1 rounded-lg border border-border"
